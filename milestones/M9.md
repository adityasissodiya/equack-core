...
EncV1 {
  ...
  aead_tag: [16],
  ct: bytes,
    // In code, AAD is derived from header fields plus the logical location:
  //   author_pk, HLC (physical/logical), parents, obj, field.
  //
  // Conceptually: aad = blake3("ECAC_AAD" || author_pk || hlc_ms || hlc_logical || parents || obj || field)
}
```
...
* **AAD discipline**
  Bind AEAD to the *header* and logical location:
  `(author_pk, hlc, parents, obj, field)` so ciphertext can’t be transplanted
  between fields or replayed under a different op header.
* **CLI viewer identity**
