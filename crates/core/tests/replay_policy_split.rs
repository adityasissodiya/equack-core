...
fn incremental_parity_with_policy_any_split() {
    ...
    for op in ops.iter().cloned() { dag_all.insert(op); }
    ...
    for split in 0..=ops.len() {
        ...
        for op in ops[..split].iter().cloned() { dag_prefix.insert(op); }
        ...
        assert_eq!(s_inc.to_deterministic_json_string(), json_full, "split={split}");
