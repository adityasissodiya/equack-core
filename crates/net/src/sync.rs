            ...
            eprintln!(
                ...
                while let Some((id, expand)) = stack.pop() {
                        ...
                        let is_head    = remote_heads.iter().any(|h| h == &id);
                                    // Include unless we already have it or bloom hints it's present, BUT never skip heads due to bloom.
                                    let include     = !has_it && (!bloom_here || is_head);
                                    #[cfg(test)]
                                    eprintln!(
